class CommentsController < ApplicationController
  ...
  def add_comment
    if user_signed_in?
      if Meetupmember.where(
           "meetup_id = ? and user_id = ?",
           params[:id],
           current_user.id
         ).first != nil
        @comment =
        ...
        Meetupcomment.create(
          { meetup_id: params[:id], comment_id: @comment.id }
        )
    ...
    if user_signed_in?
      if current_user.has_role? :disabled
        respond_to do |format|
          format.html do
            redirect_to home_index_path, alert: "Your account has been disabled"
          end
        end
      end
    end
